...
def _mk_bucketmapper(n : int = 0, 
					keys : list[int] = list()
					) -> BucketMapper[_TestItem]:
...
def test_2_get(test_bucketmapper : BucketMapper[_TestItem], 
				test_keys: list[int],
				) -> None:
	...
	for key in test_keys:
		if (item := bm.get(key)) is not None:
			...
			assert item.key == min([ x for x in bm_keys if x > key ]), "get() return too large key"
		else:
			assert key >= max(bm_keys), "get() did not return key even one existed"
	

def test_3_get_fails(test_bucketmapper : BucketMapper[_TestItem], 
					) -> None:
	bm : BucketMapper[_TestItem] = test_bucketmapper
	bm_keys : list[int] = _get_keys(bm)
...
def test_4_pop(test_bucketmapper : BucketMapper[_TestItem], 
				) -> None:
	bm : BucketMapper[_TestItem] = test_bucketmapper
	...
	for _ in range(LOOPS):
		...
		if (item := items.pop(ndx)) is None:
			...
			if (bm_item := bm.pop(item=item)) is None:
				assert item.key > max([ i.key for i in bm.list()]), "pop() did not return an item even one exists"
			else:
				assert bm_item.key >= item.key, "pop() returned wrong item"


def test_5_pop_key(test_bucketmapper : BucketMapper[_TestItem],
					test_keys : list[int]) -> None:
	bm : BucketMapper[_TestItem] = test_bucketmapper
	# items : list[_TestItem] = bm.list()		
	for key in test_keys:
		# ndx : int = randrange(len(items))
		...
		if (bm_item := bm.pop(key=key)) is None:
			assert key > max([ i.key for i in bm.list()]), "pop() returned None, even larger keys exist"
		else:
			assert bm_item.key >= key, f"pop() returned wrong item bm.keys: {[k.key for k in bm.list()]}"